...
add_action( 'after_plugin_row_' . plugin_basename( dirname( __DIR__ ) . '/plugin.php' ), array( 'WPRXTodoListPlugin', 'status' ) );
...
if ( is_dir( dirname( __DIR__ ) . '/extensions' ) ) {
	add_filter( 'mwp_framework_extension_dirs', function( $dirs ) {
		$dirs[] = array( 'namespace' => 'WPRX\TodoList\Extensions', 'path' => dirname( __DIR__ ) . '/extensions' );
		return $dirs;
	});
}
